...
'''
optimizer = optim.Adam(modelbin_ebp.parameters(), lr=1e-5)
optimizer = optim.Adam(modelbin_mvgRelax.parameters(), lr=1e-5)
optimizer = optim.Adam(modelbin_mvg.parameters(), lr=1e-5)

for epoch in range(1, args.epochs + 1):

    traincorr_avg_EBPrelaxed[epoch - 1, l, dr] = train(epoch,modelbin_ebp)
    f, testcorr_avg_EBPrelaxed[epoch - 1,l,dr] = test(epoch,modelbin_ebp)
    testcorr_avg_MVGrelaxed[epoch - 1, l, dr] = train(epoch,modelbin_mvgRelax)
    f, testcorr_avg_MVGrelaxed[epoch - 1,l,dr] = test(epoch,modelbin_mvgRelax)
    traincorr_avg_MVG[epoch - 1, l, dr] = train(epoch, modelbin_mvg)
    f, testcorr_avg_MVG[epoch - 1, l, dr] = test(epoch, modelbin_mvg)
